// This file is catctl's `probe` verb (formerly the standalone cmd/wsprobe):
// a stdlib-only WebSocket client for the WS9 browser protocol
// (internal/browserproto), used to verify cmd/catway end-to-end without a
// browser. Unlike every other verb it dials the catway's /ws endpoint — the
...
func runProbe(args []string) int {
	fs := flag.NewFlagSet("catctl probe", flag.ContinueOnError)
	rawURL := fs.String("url", "ws://localhost:8421/ws", "catway WebSocket URL")
	cols := fs.Int("cols", 120, "grid columns")
	rows := fs.Int("rows", 32, "grid rows")
	script := fs.String("script", "wait:1000; dump:0", "op script (see cmd/catctl/probe.go doc comment)")
...
func probeRun(rawURL string, cols, rows int, script string, timeout, life time.Duration, token string, viewer bool) error {
	...
	if len(p.errs) > 0 {
		fmt.Printf("catway errors seen (non-fatal): %v\n", p.errs)
	}
	return nil
}
...
func (p *probe) exec(op string, timeout time.Duration) error {
	...
	case "serverstop":
		// server.stop acks, then the catway broadcasts shutdown and exits ~250ms
